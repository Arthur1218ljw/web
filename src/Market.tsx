...
function Market() {
  ...
  const [inputView, setInputView] = useState(<div></div>);
  const [record, setRecord] = useState({image:'',title:'',deadline:0,sn:'',txid:'',amount:0,id:0});
  ...
  async function getAuctions() {
    ...
    fetch(process.env.REACT_APP_APIHOST+"/api/index/markets",
    {
  ...
  return (
    <div>
        {contextHolder}
        {contentView}
        <Modal title={record.title} open={isModalOpen} onOk={handleOk} onCancel={handleCancel} okText="Bid" style={{textAlign:'center'}}>
          <div>
